...
export default function fates(roll, party, inventory) {
  ...
  if (roll <= 10) {
    if (party.members[charIndex].stamina > 0) {
      ...
      $("randomEventMessage").text(
        `Your party gets robbed in the middle of the night... looks like they stole ${bulletVar} bullets, ${foodVar} foods, and ${medkitVar} medkits. Darn.`
      );
      ...
      $("randomEventMessage").text(
        party.party[charIndex].name +
          `ate rotten food and couldnt sleep at night lost ${healthVar} health and ${staminaVar} stamina`
      );
      ...
      $("randomEventMessage").text(
        `The party gets caught in a firefight with other survivors, ${party.party[charIndex].name} gets nicked by a bullet and loses ${healthVar}.`
      );
      party.members[charIndex].health -= healthVar;
    } else {
